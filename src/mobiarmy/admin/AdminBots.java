...
import mobiarmy.server.BotLoadout;
...
public final class AdminBots {
    public record Row(int id, String name, int glass, int level, String state, String room,
                      String targetMode, boolean locked, BotPolicy policy, String decision, String metrics, String inventory) {
        public Row(int id,String name,int glass,int level,String state,String room,String targetMode,boolean locked) {
            this(id,name,glass,level,state,room,targetMode,locked,BotPolicy.defaults(),"","","");
        }
    }
    ...
    static synchronized String submit(AdminAccounts.Account actor, Map<String,String> form) {
        ...
        if (!Set.of("create", "leave", "remove", "target", "policy", "loadout").contains(action))
        ...
        if (action.equals("policy")) mode = JSON.toJson(BotPolicy.fromForm(form));
        if (action.equals("loadout")) {
            mode = form.getOrDefault("slots", "0,0,1,1");
            exp = Integer.parseInt(form.getOrDefault("quantity", "0"));
            if (mode.length()>64 || exp<0 || exp>99) throw new IllegalArgumentException("Bộ item/số lượng không hợp lệ");
        }
    ...
    static void validateCurrent(Request r) {
        if (r.action().equals("create")) {
            ...
            if (!Set.of("target","policy").contains(r.action()) && (b.lock || b.roomWait != null && b.roomWait.started))
                throw new IllegalArgumentException("Bot đang trong trận hoặc xử lý hành động; hãy thử sau khi kết thúc");
            if (r.action().equals("loadout")) BotLoadout.validate(b,r.mode(),r.exp());
            if (r.action().equals("policy")) JSON.fromJson(r.mode(),BotPolicy.class);
        }
    }
    static String apply(Request r) {
        ...
        switch (r.action()) {
            ...
            case "policy" -> {
                b.policyOverride = JSON.fromJson(r.mode(),BotPolicy.class);
                return "Đã đặt cấu hình bot " + b.id + "; áp dụng từ lượt tiếp theo: " + r.mode();
            }
            case "loadout" -> {
                BotLoadout.apply(b,r.mode(),r.exp());
                return "Đã đặt 4 slot " + r.mode() + ", cấp thêm " + r.exp() + " mỗi loại item đặc biệt; " + BotLoadout.describe(b);
            }
            case "target" -> {
    ...
    private static Row row(Bot b) {
        return new Row(b.id, b.name, b.selectGlass, b.glass().level,
                ...
                b.effectiveTargetMode().name() + (b.targetModeOverride == null ? " (mặc định)" : ""), b.lock, b.effectivePolicy(), b.lastDecision,
                "Quyết định="+b.decisions+" · Đoạn đi="+b.moves+" · Item="+b.usedItems
                +" · Tìm góc="+b.searchCalls+" · ms="+(b.searchNanos/1_000_000)+" · Timeout="+b.timeouts+" · Bỏ lượt="+b.skippedTurns,
                BotLoadout.describe(b));
    }
